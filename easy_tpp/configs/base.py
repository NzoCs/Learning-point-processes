...
class ConfigValidator:
    ...
    def validate_required_fields(
        self, config: "BaseConfig", required_fields: List[str]
    ) -> None:
...
class BaseConfig(Registrable, ABC):
    ...
    def load_from_yaml_file(
        cls: Type[ConfigType], file_path: Union[str, Path]
    ) -> ConfigType:
        """
...
class ConfigFactory:
    ...
    def register_config_class(
        self, config_type: str, config_class: Type[BaseConfig]
    ) -> None:
        """Register a configuration class."""
        self._config_registry[config_type] = config_class
        logger.debug(
            f"Registered config class: {config_class.__name__} for type: {config_type}"
        )
    ...
    def create_config(
        ...
        try:
            ...
            raise ConfigValidationError(
                f"Failed to create {config_type} configuration: {str(e)}"
            )

    def load_config_from_file(
        self, config_type: str, file_path: Union[str, Path]
    ) -> BaseConfig:
        """
