...
class Visualizer:
    def __init__(
        ...
        self.data = self.data_module.build_input(
            source_dir=data_dir, data_format=data_format, split=split
        )
        ...
        i = 0
        for type_seq, time_seq in zip(
            self.data["type_seqs"], self.data["time_delta_seqs"]
        ):
        ...
        if self.is_comparison_mode:
            ...
            comp_data_dir = comparison_data_module.data_config.get_data_dir(
            ...
            i = 0
            for type_seq, time_seq in zip(
                self.comparison_data["type_seqs"],
                self.comparison_data["time_delta_seqs"],
            ):
                for event, time in zip(type_seq, time_seq):
            ...
            self.comparison_seq_lengths = np.array(
                [len(seq) for seq in self.comparison_data["type_seqs"]]
            )
    ...
    def _save_metadata_to_json(self, metadata, filename_base):
        ...
        if "plot_type" in metadata:
            ...
            if (
                metadata["plot_type"] == "delta_times_distribution"
                and "data_points" not in metadata
            ):
                ...
                metadata["plot_type"] == "marked_delta_times_distribution"
                and "data_points" not in metadata
            ):
                ...
                for event_type in range(self.num_event_types):
                    data = [
                        self.all_time_deltas[i]
                        for i, t in enumerate(self.all_event_types)
                        if t == event_type
                    ]
                    if len(data) > 5000:
                ...
                metadata["plot_type"] == "event_type_distribution"
                and "raw_counts" not in metadata
            ):
                # Store raw counts of each event type
                event_counts = {
                    str(k): int(v) for k, v in Counter(self.all_event_types).items()
                }
                metadata["raw_counts"] = event_counts

            elif (
                metadata["plot_type"] == "seq_len_distribution"
                and "sequence_lengths" not in metadata
            ):
                ...
                if "raw_data" not in metadata:
                    metadata["raw_data"] = {
                        "delta_times_sample": (
                            self.all_time_deltas[:5000]
                            if len(self.all_time_deltas) > 5000
                            else self.all_time_deltas
                        ),
                        "is_sample": len(self.all_time_deltas) > 5000,
                        "event_type_counts": {
                            str(k): int(v)
                            for k, v in Counter(self.all_event_types).items()
                        },
                        "sequence_lengths": (
                            [len(seq) for seq in self.data["type_seqs"]][:1000]
                            if len(self.data["type_seqs"]) > 1000
                            else [len(seq) for seq in self.data["type_seqs"]]
                        ),
                    }
    ...
    def delta_times_distribution(self, filename: str = "inter_event_time_dist.png"):
        ...
        if self.is_comparison_mode and len(filtered_comp_hist) > 1:
            ...
            comp_x_line = np.linspace(
                min(filtered_comp_bin_centers), max(filtered_comp_bin_centers), 100
            )
    ...
    def _create_qq_plot(
        ...
        if len(data1) == 0 or len(data2) == 0:
            print(
                f"Warning: Empty arrays after filtering: data1={len(data1)}, data2={len(data2)}"
            )
            return
        ...
        if log_scale:
            ...
            min_val = max(
                min(data1_quantiles.min(), data2_quantiles.min()), 1e-10
            )  # Avoid log(0)
    ...
    def event_type_distribution(self, filename: str = "event_type_dist.png"):
        ...
        if self.is_comparison_mode:
            ...
            comp_type_counts = np.bincount(
                comp_types_int, minlength=self.num_event_types
            )

            comp_total = len(comp_types_data)
            comp_type_probs = (
                comp_type_counts / comp_total
                if comp_total > 0
                else np.zeros_like(comp_type_counts)
            )
            ...
            plt.bar(
                ...
                comp_type_probs,
                width,
                label=f"Comparison ({self.comparison_split})",
                color="crimson",
                alpha=0.7,
            )
        ...
        main_stats = "Main Top Types:\n" + "\n".join(
            [f"Type {all_event_types[i]}: {type_probs[i]:.3f}" for i in top3_indices]
        )
        ...
        if self.is_comparison_mode:
            ...
            comp_stats = "Comparison Top Types:\n" + "\n".join(
                [
                    f"Type {all_event_types[i]}: {comp_type_probs[i]:.3f}"
    ...
    def sequence_length_distribution(self, filename: str = "sequence_length_dist.png"):
        ...
        # Calculate bin width
        binwidth = (
            max(1, int(np.std(self.seq_lengths) / 2))
        ...
        if self.is_comparison_mode:
            comp_binwidth = (
                max(1, int(np.std(self.comparison_seq_lengths) / 2))
                if len(self.comparison_seq_lengths) > 1
                else 1
            )
