...
from data_gen.base_simulator import BaseSimulator
...
class HawkesSimulator(BaseSimulator):
    ...
    def simulate(self) -> Tuple[List[np.ndarray]]:
        ...
        with tqdm(total=self.end_time - self.start_time, 
                  desc=f"Simulation processus Hawkes {dim}D",
                  unit="time") as pbar:
            ...
            while t < self.end_time:
                ...
                pbar.update(min(dt, self.end_time - pbar.n - self.start_time))
        
        # Conversion en tableaux numpy
        self.simulations = tuple(np.array(events_dim) for events_dim in events)
