...
This module provides data extraction classes following the Single Responsibility Principle (SRP).
Each extractor is responsible for extracting data from a specific source.
...
class TPPDatasetExtractor:
    ...
    def _extract_all_data(self) -> None:
        ...
        """
        logger.info(f"Extracting ground truth data from TPPDataset with {len(self.dataset)} sequences...")
        ...
        for idx in range(dataset_len):
            ...
            try:
                ...
                if isinstance(type_seq, np.ndarray):
                    ...
                    time_deltas = time_delta_seq[valid_mask] if isinstance(time_delta_seq, np.ndarray) else time_delta_seq
        ...
        logger.info(f"Successfully processed {processed_sequences}/{dataset_len} sequences, "
...
class LabelDataExtractor:
    ...
    def _process_batch(self, batch) -> bool:
        ...
        try:
            time_delta_seqs, type_seqs, attention_mask = self._extract_batch_tensors(batch)
    ...
    def _extract_batch_tensors(self, batch) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor]]:
        """
    ...
    def _process_sequence(self, time_deltas: torch.Tensor, event_types: torch.Tensor, attention_mask: Optional[torch.Tensor]) -> bool:
        """
