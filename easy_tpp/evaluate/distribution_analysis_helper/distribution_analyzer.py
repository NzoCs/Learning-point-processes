...
This module provides statistical analysis and visualization utilities following
the Single Responsibility Principle (SRP). It focuses solely on statistical analysis.
...
class DistributionAnalyzer:
    ...
    def plot_density_comparison(
        ...
        try:
            ...
            all_data = np.concatenate([ds['data'] for ds in datasets if len(ds['data']) > 0])
            if len(all_data) == 0:
    ...
    def _add_regression_analysis(
        ...
        try:
            ...
            if len(x_filtered) <= 1:
                logger.warning(f"Insufficient data points for regression analysis in {dataset['label']}")
                return None
                
            # Perform linear regression on log-transformed density
            slope, intercept, r_value, _, _ = linregress(x_filtered, np.log10(y_filtered))
            ...
            logger.warning(f"Regression analysis failed for {dataset['label']}: {str(e)}")
            return None

    @staticmethod
    def _add_statistical_summary(dataset: Dict[str, Any], ax, total_datasets: int) -> None:
        """
        ...
        try:
            ...
            is_ground_truth = any(keyword in dataset['label'].lower() 
            ...
            logger.warning(f"Failed to add statistical summary for {dataset['label']}: {str(e)}")
    ...
    def create_qq_plot(
        ...
        try:
            ...
            if log_scale:
                plt.loglog(ref_quantiles, comp_quantiles, 'o', 
                          markersize=4, color='royalblue', alpha=0.7)
                ...
                plt.plot(ref_quantiles, comp_quantiles, 'o', 
                        markersize=4, color='royalblue', alpha=0.7)
