...
This module provides metrics calculation functionality following
        ...
        try:
            ...
            if len(label_deltas) > 0:
                metrics.update({
                    ...
                    "ground_truth_median_inter_event_time": float(np.median(label_deltas)),
                    "ground_truth_std_inter_event_time": float(np.std(label_deltas)),
                })
            
            if len(simulated_deltas) > 0:
                metrics.update({
                    "simulation_mean_inter_event_time": float(np.mean(simulated_deltas)),
                    "simulation_median_inter_event_time": float(np.median(simulated_deltas)),
            ...
            if len(label_lengths) > 0:
                metrics.update({
                    ...
                    "ground_truth_median_sequence_length": float(np.median(label_lengths)),
                })
            
            if len(simulated_lengths) > 0:
                metrics.update({
                    "simulation_mean_sequence_length": float(np.mean(simulated_lengths)),
                    "simulation_median_sequence_length": float(np.median(simulated_lengths)),
                })
            ...
            if len(label_deltas) > 0 and len(simulated_deltas) > 0:
                ...
                metrics["relative_mean_time_difference"] = float((sim_mean - gt_mean) / gt_mean)
