...
following the Single Responsibility Principle. Now refactored with separate
extractors for types and times.
"""
...
from .interfaces import MetricsComputerInterface, DataExtractorInterface, TimeExtractorInterface, TypeExtractorInterface
...
class PredictionDataExtractor(DataExtractorInterface):
    """Extracts prediction data from batch and predictions - legacy compatibility."""
    ...
    def extract_values(self, batch: Any, pred: Any) -> MaskedValues:
        ...
        logger.debug(f"DEBUG: Batch type: {type(batch)}, length: {len(batch) if hasattr(batch, '__len__') else 'N/A'}")
        logger.debug(f"DEBUG: Pred type: {type(pred)}, length: {len(pred) if hasattr(pred, '__len__') else 'N/A'}")
...
class PredictionMetricsComputer(MetricsComputerInterface):
    ...
    def __init__(
        ...
        if selected_metrics is None:
            ...
            if invalid_metrics:
                logger.warning(f"Invalid prediction metrics requested: {invalid_metrics}. "
        ...
        try:
            ...
            if self.num_event_types > 1:
                ...
                if PredictionMetrics.CONFUSION_MATRIX.value in self.selected_metrics:
                    metrics['confusion_matrix'] = self._calculate_confusion_matrix(masked)
    ...
    def compute_all_time_metrics(self, batch: Any, pred: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics['time_rmse'] = self._calculate_time_rmse_from_time_values(time_values)
    ...
    def compute_all_type_metrics(self, batch: Any, pred: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.num_event_types > 1:
                ...
                metrics['type_accuracy'] = self._calculate_type_accuracy_from_type_values(type_values)
                metrics['macro_f1score'] = self._calculate_f1_score_from_type_values(type_values, average='macro')
                metrics['recall'] = self._calculate_recall_from_type_values(type_values)
                metrics['precision'] = self._calculate_precision_from_type_values(type_values)
                metrics['cross_entropy'] = self._calculate_cross_entropy_from_type_values(type_values)
                metrics['confusion_matrix'] = self._calculate_confusion_matrix_from_type_values(type_values)
            else:
                ...
                metrics = {
                    ...
                    'confusion_matrix': torch.full((self.num_event_types, self.num_event_types), float('nan'))
                }
            ...
            return {
                ...
                'recall': float('nan'),
                'precision': float('nan'),
                'cross_entropy': float('nan'),                'confusion_matrix': torch.full((self.num_event_types, self.num_event_types), float('nan'))
            }
    ...
    def get_available_metrics(self) -> List[str]:
        ...
        if self.num_event_types > 1:
            metrics.extend([
                'type_accuracy', 'macro_f1score', 
                'recall', 'precision', 'cross_entropy', 'confusion_matrix'
            ])
        return metrics
    ...
    def _calculate_time_rmse_from_time_values(self, time_values: TimeValues) -> float:
        ...
        if time_values.true_times.numel() == 0:
            return float('nan')
        return torch.sqrt(F.mse_loss(time_values.pred_times, time_values.true_times)).item()
    ...
    def _calculate_confusion_matrix(self, masked: MaskedValues) -> torch.Tensor:
        ...
        if self.num_event_types <= 1 or masked.true_types.numel() == 0:
            return torch.full((self.num_event_types, self.num_event_types), float('nan'))
    ...
    def _calculate_type_accuracy_from_type_values(self, type_values: TypeValues) -> float:
    ...
    def _calculate_f1_score_from_type_values(self, type_values: TypeValues, average: str = 'macro') -> float:
    ...
    def _calculate_cross_entropy_from_type_values(self, type_values: TypeValues) -> float:
        ...
        if type_values.pred_types.dim() == 1:
            pred_logits = F.one_hot(type_values.pred_types, num_classes=self.num_event_types).float()
        else:
    ...
    def _calculate_confusion_matrix_from_type_values(self, type_values: TypeValues) -> torch.Tensor:
