...
class Benchmarks(Enum):
    ...
    INTERTIME_DISTRIBUTION = ("intertime_distribution_sampling", InterTimeDistributionBenchmark)
    ...
    def get_benchmark_by_name(cls, name: str) -> 'Benchmarks':
        ...
        for benchmark in cls:
            if benchmark.benchmark_name == name:
                return benchmark
        raise ValueError(f"Benchmark '{name}' introuvable. Disponibles: {[b.benchmark_name for b in cls]}")
...
class BenchmarkManager:
    ...
    def __init__(self, data_config, dataset_name: str, save_dir: Optional[Union[Path, str]] = None):
        """
