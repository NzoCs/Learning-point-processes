...
class ModelRegistry:
    ...
    def _discover_all_models(self):
            ...
            try:
                ...
                for name in dir(module):
                    ...
                    if (
                        ...
                        logger.debug(
                            f"Modèle découvert: {obj.__name__} dans {module_name}"
                        )
        ...
        logger.info(
            f"Découverte terminée: {len(self._discovered_models)} modèles trouvés"
        )
    ...
    def generate_enum_code(self) -> str:
        ...
        for class_name, model_class in sorted(self._discovered_models.items()):
            module_name = model_class.__module__.split(".")[
                -1
            ]  # Dernier partie du module
        ...
        if bases and any(issubclass(base, BaseModel) for base in bases):
            ...
            if cls != BaseModel:
                ...
                try:
                    ...
                    model_exists = any(
                        model.get_class_name() == name for model in Models
                    )

                    if not model_exists:
                        logger.info(f"🔄 Nouveau modèle détecté: {name}")
                        logger.warning(
                            f"⚠️ Le modèle '{name}' n'est pas dans l'enum Models"
                        )
                    ...
                    logger.debug(
                        f"Erreur lors de la vérification du modèle {name}: {e}"
                    )
