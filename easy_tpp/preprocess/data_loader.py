...
class TPPDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit':
            ...
            logger.info(f"Train dataset created with {len(self.train_dataset)} sequences")
            
            val_data_dir = self.data_config.get_data_dir('dev')
            self.val_data = self.build_input(val_data_dir, self.data_config.data_format, 'dev')
    ...
    def get_dataloader(self, split: str, **kwargs):
        ...
        if split == 'train':
            ...
            raise ValueError(f"Unrecognized split: {split}. Must be one of 'train', 'dev'/'val', 'test', or 'predict'")
