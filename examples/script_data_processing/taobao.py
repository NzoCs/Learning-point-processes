...
def check_dominate_event_type(event_type_seq, threshold=0.7):
    ...
    type_ratio = [
        len(event_type_seq[event_type_seq == event_type_i]) / total_len
...
def cate_map(cate_id, cate_event_map_df):
    res = cate_event_map_df[cate_event_map_df["cate"] == cate_id]["event_id"].to_list()[
        0
    ]
    return res
...
def read_data_step_3(source_dir, cate_dir, target_dir):
    ...
    train_df["event_type"] = train_df["cate_id"].apply(
        lambda x: cate_map(x, cate_event_map_df)
    )
    ...
    for idx, user_id in enumerate(unique_user_id):
        ...
        assert (
            abs(
                np.mean(user_df["event_time"].diff().values[1:])
                - np.mean(event_dtime[1:])
            )
            < 0.0001
        )
...
def save_data(source_dir):
    ...
    for idx, user_id in enumerate(unique_user_id):
        ...
        for idx_row, row in user_df.iterrows():
            ...
            user_seq.append(
                {
                    "time_since_last_event": event_dtime,
    ...
    with open("../data/taobao/train.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 17, "train": res[:1300]}, f_out)

    with open("../data/taobao/dev.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 17, "dev": res[1300:1500]}, f_out)

    with open("../data/taobao/test.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 17, "test": res[1500:]}, f_out)
