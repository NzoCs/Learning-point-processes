...
def read_data_step_1():
    ...
    with open("../data/taxi/train.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 10, "train": train_res[:1500]}, f_out)

    with open("../data/taxi/dev.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 10, "dev": dev_res[:200]}, f_out)

    with open("../data/taxi/test.pkl", "wb") as f_out:
        pickle.dump({"dim_process": 10, "test": test_res[:400]}, f_out)
