...
def make_datetime(year, month, day):
    ...
    return (
        datetime.datetime.timestamp(date) + 61851630000
    )  # make sure the timestamp is positive
...
def clean_csv():
    ...
    df["event_timestamp"] = (
        df.apply(
            lambda x: make_datetime(
                x["event_date_year"], x["event_date_month"], x["event_date_day"]
            ),
            axis=1,
        )
        / norm_const
    ...
    df.sort_values(
        by=["event_date_year", "event_date_month", "event_date_day"], inplace=True
    )
...
def make_seq(df):
    ...
    for index, row in df.iterrows():
        if index == 0:
            ...
            event_dict = {
                "time_since_last_event": row["time_diff"],
...
def make_pkl(target_dir, dim_process, split, seqs):
    with open(target_dir, "wb") as f_out:
        pickle.dump({"dim_process": dim_process, split: seqs}, f_out)
    return
