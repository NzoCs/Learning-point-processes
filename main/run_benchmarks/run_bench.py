...
class BenchmarkRunner:
    ...
    def __init__(
        self, config_path: str = None, output_dir: str = None, debug: bool = False
    ):
        """
        ...
        logger.info(
            f"Found {len(self.datasets)} datasets: {list(self.datasets.keys())}"
        )
    ...
    def _create_data_config(self, dataset_name: str) -> DataConfig:
        ...
        dataset_config["data_loading_specs"] = dataset_config.get(
            "data_loading_specs",
            {"batch_size": 32, "num_workers": 1, "tensor_type": "pt"},
        )
    ...
    def run_benchmark(self, dataset_name: str, benchmark_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.debug:
                logger.info(
                    f"Debug mode enabled: Detailed error information will be shown"
                )
            ...
            logger.info(
                f"✅ {benchmark_name} benchmark completed successfully on {dataset_name}"
            )
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"❌ {benchmark_name} benchmark failed on {dataset_name}: {str(e)}"
            )
    ...
    def run_all_benchmarks_on_dataset(
        ...
        for benchmark_name in self.AVAILABLE_BENCHMARKS.keys():
            try:
                ...
                logger.error(
                    f"Failed to run {benchmark_name} on {dataset_name}: {str(e)}"
                )
    ...
    def run_benchmark_on_all_datasets(
        ...
        logger.info(
            f"✅ Completed {benchmark_name} benchmark on {len(all_results)} datasets"
        )
        if failed_datasets:
    ...
    def run_all_benchmarks_on_all_datasets(
        self,
        ...
        for dataset_name in self.datasets.keys():
            ...
            for benchmark_name in self.AVAILABLE_BENCHMARKS.keys():
                ...
                logger.info(
                    f"Progress: {completed_runs + failed_runs}/{total_runs} runs completed"
                )
...
def print_datasets(runner: BenchmarkRunner):
    ...
    for dataset_name in datasets:
        ...
        print(
            f"  • {dataset_name:<15} | Format: {data_format:<6} | Event Types: {num_event_types}"
        )
    print()
...
Exemples d'utilisation:
  ...
  %(prog)s --config custom_config.yaml --output ./results  # Utiliser une config personnalisée
    ...
    parser.add_argument(
        "--list-benchmarks", action="store_true", help="List all available benchmarks"
    )  # Execution options
    parser.add_argument(
        "--dataset",
        "-d",
    ...
    parser.add_argument("--benchmark", "-b", type=str, help="Benchmark name to run")
    parser.add_argument(
        "--all-datasets", action="store_true", help="Run on all available datasets"
    )
    parser.add_argument(
        "--all-benchmarks", action="store_true", help="Run all available benchmarks"
    )
    ...
    try:
        ...
        runner = BenchmarkRunner(
            config_path=args.config, output_dir=args.output, debug=args.debug
        )
        ...
        if args.all_datasets and dataset_specified:
            print(
                "❌ Erreur: Vous ne pouvez pas utiliser --dataset et --all-datasets en même temps"
            )
            return 1
        ...
        if args.benchmark and args.all_benchmarks:
            print(
                "❌ Erreur: Vous ne pouvez pas utiliser --benchmark et --all-benchmarks en même temps"
            )
            return 1
