...
class ModelRunner:
    ...
    def run_experiment(
        ...
        try:
            ...
            config_dict = parse_runner_yaml_config(
                str(self.config_path), experiment_id, dataset_id
            )
            ...
            exp_output_dir = os.path.join(
                self.output_dir, f"{experiment_id}_{dataset_id}_{phase}"
            )
    ...
    def run_all_experiments_on_dataset(
        self, dataset_id: str, phase: str
    ) -> Dict[str, bool]:
    ...
    def run_experiment_on_all_datasets(
        self, experiment_id: str, phase: str, checkpoint_path: str = None
    ) -> Dict[str, bool]:
        ...
        for dataset_id in datasets:
            ...
            success = self.run_experiment(
                experiment_id, dataset_id, phase, checkpoint_path
            )
            results[dataset_id] = success

        return results

    def run_all_experiments_on_all_datasets(
        self, phase: str
        ...
        logger.info(
            f"🎯 Total combinations: {len(experiments)} × {len(datasets)} = {len(experiments) * len(datasets)}"
        )
...
Exemples d'utilisation:
  ...
  python run_model.py --all-experiments --all-datasets --phase test
    ...
    group.add_argument("--experiment", type=str, help="ID de l'expérience à exécuter")
    group.add_argument(
        "--all-experiments",
        action="store_true",
    ...
    dataset_group.add_argument("--dataset", type=str, help="ID du dataset à utiliser")
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="Chemin vers le fichier checkpoint (optionnel)"
    )
    ...
    try:
        ...
        if not (args.dataset or args.all_datasets):
            print(
                "❌ Erreur: Vous devez spécifier un dataset avec --dataset ou --all-datasets"
            )
        ...
        if args.all_experiments and args.all_datasets:
            ...
            logger.info(
                f"� Summary: {successful_combinations}/{total_combinations} combinations successful"
            )
            ...
            if successful < total:
                failed_experiments = [
                    exp for exp, success in results.items() if not success
                ]
            ...
            results = runner.run_experiment_on_all_datasets(
                args.experiment, args.phase, args.checkpoint
            )
            ...
            if successful < total:
                failed_datasets = [
                    dataset for dataset, success in results.items() if not success
                ]
            ...
            success = runner.run_experiment(
                args.experiment, args.dataset, args.phase, args.checkpoint
            )
