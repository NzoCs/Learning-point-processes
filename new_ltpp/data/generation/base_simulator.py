...
class BaseSimulator(ABC):
    ...
    def format_multivariate_simulations(
        self, simulations, dim_process=None, start_time=None
    ):
        """
    ...
    def split_data(self, data, train_ratio=0.6, test_ratio=0.2, dev_ratio=0.2):
        ...
        """
        assert (
            abs(train_ratio + test_ratio + dev_ratio - 1.0) < 1e-10
    ...
    def generate_and_save(
        ...
        self._save_metadata(
            output_dir, num_simulations, data_splits, splits, formatted_data
        )
        ...
        **kwargs,
        ...
        if save_data:
            ...
            intensity_data = {
                ...
                "marked_times": {
                    str(dim): times.tolist() for dim, times in marked_times.items()
                },
                "metadata": {
                    "precision": precision,
                    "start_time": self.start_time,
        ...
        if plot or save_plot:
            ...
            fig, axes = plt.subplots(
                self.dim_process, 1, figsize=(12, 3 * self.dim_process)
            )
            ...
            for i in range(self.dim_process):
                ...
                if len(marked_times[i]) > 0:
                    ...
                    if len(events_in_window) > 0:
                        ax.scatter(
                            events_in_window,
                            np.zeros_like(events_in_window)
                            - 0.05 * intensities[:, i].max(),
                            s=30,
                            color=f"C{i}",
