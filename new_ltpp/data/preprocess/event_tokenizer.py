...
class EventTokenizer:
    ...
    def pad(
        ...
        sequences = self._truncate(
            inputs, truncation_strategy=truncation_strategy, max_length=max_length
        )

        # Apply padding (max_length will be calculated dynamically if needed)
        # Returns torch tensors directly
        batch_output = self._pad(
            sequences, padding_strategy=padding_strategy, max_length=max_length
        )
    ...
    def _pad(
        ...
        if padding_strategy == PaddingStrategy.DO_NOT_PAD or is_uniform_length:
            ...
            if max_length is None:
                raise ValueError(
                    "max_length must be specified for MAX_LENGTH padding strategy"
                )
