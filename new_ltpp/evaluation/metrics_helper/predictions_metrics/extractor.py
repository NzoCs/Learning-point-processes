...
class TimeDataExtractor():
    ...
    def extract_time_values(self, batch: Batch, pred_time_tensor: torch.Tensor) -> TimeValues:
        ...
        if batch_non_pad_mask is not None:
            ...
            true_times = true_time_delta_seqs.flatten()
            pred_times = pred_time_tensor.flatten()
...
class TypeDataExtractor():
    ...
    def extract_type_values(self, batch: Batch, pred_type_tensor: torch.Tensor) -> TypeValues:
        ...
        if batch_non_pad_mask is not None:
            ...
            true_types = true_type_seqs.flatten()
            pred_types = pred_type_tensor.flatten()
...
class PredictionDataExtractor():
    ...
    def extract_values(self, batch: Batch, pred: OneStepPrediction) -> tuple[TimeValues, TypeValues]:
