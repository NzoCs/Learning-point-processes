...
class RBFTimeKernel(ISpaceKernel):
    def __init__(self, sigma: Optional[float] = None, scaling: float = 1.0):
    ...
    def Gram_matrix(
        ...
        norm_matrix = (X - Y) ** 2
        sigma = self.sigma if self.sigma is not None else norm_matrix.median() + 1e-8  # Median heuristic for bandwidth
