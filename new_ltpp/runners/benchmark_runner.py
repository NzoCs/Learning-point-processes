    ...
    def run_benchmark(
        ...
        **benchmark_params
        ...
        try:
            ...
            data_configs_list = data_config if isinstance(data_config, list) else [data_config]
            ...
            for data_cfg in data_configs_list:
                ...
                self.print_info(f"Configuration des données: {config_paths.get('data_config_path')}")
            ...
            if len(all_data_configs) == 1:
                ...
                self.print_info(f"BenchmarkManager créé avec {len(all_data_configs)} configurations")
            
            # Déterminer quels benchmarks exécuter
            if run_all and run_all_configs and benchmark_manager.get_config_count() > 1:
                self.print_info(f"Exécution de tous les benchmarks sur {benchmark_manager.get_config_count()} configurations...")
                self.print_info(f"Datasets: {', '.join(benchmark_manager.list_datasets())}")
                ...
                if benchmark_enums:
                    if run_all_configs and benchmark_manager.get_config_count() > 1:
                        self.print_info(f"Exécution sur {benchmark_manager.get_config_count()} configurations...")
                        results = benchmark_manager.run_multiple_on_all_configs(benchmark_enums, **benchmark_params)
                    else:
                ...
                if run_all_configs and benchmark_manager.get_config_count() > 1:
                    results = benchmark_manager.run_multiple_on_all_configs(default_benchmarks, **benchmark_params)
                else:
            ...
            if isinstance(results, dict):
                ...
                if is_multi_config:
                    num_benchmarks = sum(len(dataset_results) for dataset_results in results.values() if isinstance(dataset_results, dict))
                else:
                    num_benchmarks = len(results)
            else:
                num_benchmarks = 1
            
            self.print_success(f"Benchmarks terminés - {num_benchmarks} benchmarks exécutés")
    ...
    def _display_benchmark_results(self, results: Dict[str, Any], output_dir: str):
        ...
        if results:
            ...
            is_multi_config = isinstance(first_value, dict) and any(
                key in ['mean_inter_time', 'lag1_mark_benchmark', 'intertime_distribution_sampling', 'mark_distribution_sampling']
        ...
        if is_multi_config:
            ...
            if self.console:
                ...
                table = Table(title="Résultats des Benchmarks TPP - Multi-Configurations")
        ...
        try:
            ...
            with open(report_file, 'w') as f:
                ...
                if is_multi_config:
                    ...
                    f.write(f"Nombre de configurations: {results_data['num_configs']}\n")
                    f.write(f"Datasets: {', '.join(results_data['datasets'])}\n")
                    f.write(f"Total benchmarks exécutés: {results_data['total_benchmarks_executed']}\n")
