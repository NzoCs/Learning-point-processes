...
from new_ltpp.utils.misc import save_json


def generate_synthetic_data(
    n_nodes=3, end_time=1000, baseline=0.1, adjacency=0.5, decay=1.0
):
    """
        ...
        for i in range(n_nodes):
            for j in range(n_nodes):
                if events[j]:
                    ...
                    intensities[i] += adjacency_matrix[i, j] * np.exp(
                        -decay * (current_time - last_event_time)
                    )
...
def format_tick_data_to_hf(events, dim_process, max_seq_len):
    ...
    for seq_idx in range(0, len(all_events), max_seq_len):
        ...
        time_since_start = [
            event["time_since_start"] - start_time for event in seq_events
        ]
...
def format_gen_data_to_hf(sample_data, dim_process, max_seq_len=None):
    ...
    for batch_idx, (type_seq_batch, time_delta_seq_batch, time_seq_batch) in enumerate(
        zip(type_seqs, time_delta_seqs, time_seqs)
    ):
        # Get batch dimension
...
def generate_and_save_json(
    n_nodes, end_time, baseline, adjacency, decay, max_seq_len, target_file
):
    """
    ...
    formatted_data = format_tick_data_to_hf(
        events, dim_process=n_nodes, max_seq_len=max_seq_len
    )
