...
def test_mean_benchmark_integration(real_data_config, tmp_path):
    bench = MeanInterTimeBenchmark(real_data_config, "NzoCs_test_dataset", save_dir=str(tmp_path))
    bench.evaluate()
...
def test_mark_distribution_benchmark_integration(real_data_config, tmp_path):
    bench = MarkDistributionBenchmark(real_data_config, "NzoCs_test_dataset", save_dir=str(tmp_path))
    bench.evaluate()
    _check_json_file_created(str(tmp_path), "NzoCs_test_dataset", bench)

def test_intertime_distribution_benchmark_integration(real_data_config, tmp_path):
    bench = InterTimeDistributionBenchmark(real_data_config, "NzoCs_test_dataset", save_dir=str(tmp_path))
    bench.evaluate()
    _check_json_file_created(str(tmp_path), "NzoCs_test_dataset", bench)

def test_last_mark_benchmark_integration(real_data_config, tmp_path):
    bench = LastMarkBenchmark(real_data_config, "NzoCs_test_dataset", save_dir=str(tmp_path))
    bench.evaluate()
    _check_json_file_created(str(tmp_path), "NzoCs_test_dataset", bench)
