...
class TestDeviceConsistency:
    ...
    def test_batch_data_device_consistency(self, sample_batch_data):
        ...
        for tensor in sample_batch_data:
            if isinstance(tensor, torch.Tensor):
                ...
                check_tensor_device(tensor_on_cpu, cpu_device) @ pytest.mark.skipif(
    ...
    def test_model_forward_device_consistency(
    ...
    @pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
    def test_model_forward_gpu_consistency(
        self, sample_model_config, sample_batch_data
    ):
    ...
    def test_device_switching_during_training(
        self, sample_model_config, sample_batch_data
    ):
    ...
    @pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
    def test_gpu_gradient_device_consistency(
        self, sample_model_config, sample_batch_data
    ):
    ...
    def test_mixed_precision_device_consistency(
        self, sample_model_config, sample_batch_data
    ):
    ...
    def test_dataloader_device_consistency(self, sample_batch_data):
        ...
        for batch in mock_dataloader:
            # Move all tensors in the tuple to CPU and check
            cpu_batch = tuple(
                tensor.to(cpu_device) if isinstance(tensor, torch.Tensor) else tensor
                for tensor in batch
            )
