...
class TestRunnerTrainingSteps:
    ...
    def _create_training_config(self, model_id="NHP", temp_dir=None):
        ...
        config_dict = {
            ...
            f"{model_id}_train": {
                "data_loading_specs": {"batch_size": 4, "num_workers": 1},
                "model_config": {
                    "model_id": model_id,
                    "specs": (
                        {"hidden_size": 16, "time_emb_size": 8, "num_layers": 1}
                    ...
                    "base_config": (
                        {"lr": 0.01, "lr_scheduler": False}
                        if model_id in ["NHP", "RMTPP"]
                        else None
                    ),
                    "simulation_config": (
                        {"start_time": 10, "end_time": 20, "max_sim_events": 50}
                        if model_id in ["NHP", "RMTPP"]
                        else None
                    ),
                },
    ...
    def test_testing_step_execution(self, temporary_directory, model_id):
        ...
        with patch("easy_tpp.preprocess.data_loader.TPPDataModule.setup") as mock_setup:
            with patch(
                ...
                with patch(
                    "pytorch_lightning.Trainer.test", return_value=mock_test_results
                ) as mock_test:
    ...
    def test_checkpoint_saving_during_training(self, temporary_directory):
        ...
        with patch("easy_tpp.preprocess.data_loader.TPPDataModule.setup"):
            # Build config and create trainer
            config = RunnerConfig.load_from_yaml_file(
                yaml_path=config_path, experiment_id="NHP_train", dataset_id="test_data"
            )
                                    ...
                                    with patch("builtins.open", create=True):
                                        with patch("json.dump"):
                                            with patch(
                                                ...
                                                # Build config and create trainer
                                                config = (
                                                    RunnerConfig.load_from_yaml_file(
                                                        yaml_path=config_path,
                                                        experiment_id="NHP_train",
                                                        dataset_id="test_data",
                                                    )
                                                ...
                                                trainer.model.format_and_save_simulations = (
                                                    MagicMock()
                                                )
                                                trainer.model.simulations = MagicMock()
