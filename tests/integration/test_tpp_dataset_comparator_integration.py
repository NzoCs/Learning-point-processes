...
class TestTPPDatasetComparatorIntegration:
    ...
    def test_tpp_dataset_factory_creation(self, sample_tpp_dataset, sample_simulation, temp_output_dir):
    ...
    def test_tpp_dataset_data_extraction(self, sample_tpp_dataset, sample_simulation, temp_output_dir):
        ...
        assert np.all(event_types >= 1) and np.all(event_types <= 3)  # Event types in range
    
    def test_performance_improvement_indication(self, sample_tpp_dataset, sample_simulation, temp_output_dir):
        ...
        assert extraction_time < 1.0, f"Extraction took too long: {extraction_time:.4f} seconds"
    
    def test_backward_compatibility_with_non_tpp_dataset(self, sample_simulation, temp_output_dir):
