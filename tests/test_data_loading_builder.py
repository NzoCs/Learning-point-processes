...
def test_data_loading_specs_builder():
    ...
    print(
        f"   Batch size: {config_dict.get('data_loading_specs', {}).get('batch_size')}"
    )
    print(
        f"   Num workers: {config_dict.get('data_loading_specs', {}).get('num_workers')}"
    )
    print(f"   Shuffle: {config_dict.get('data_loading_specs', {}).get('shuffle')}")
    print(
        f"   Event types: {config_dict.get('tokenizer_specs', {}).get('num_event_types')}"
    )
    ...
    builder3.set_data_loading_specs({"batch_size": 16, "pin_memory": True})
