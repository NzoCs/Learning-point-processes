...
def test_data_config_minimal():
    config = DataConfig(train_dir='train.csv', data_specs=TokenizerConfig(num_event_types=2))
...
def test_data_config_get_data_dir():
    config = DataConfig(train_dir='train.csv', valid_dir='valid.csv', test_dir='test.csv', source_dir='all.csv', data_specs=TokenizerConfig(num_event_types=2))
