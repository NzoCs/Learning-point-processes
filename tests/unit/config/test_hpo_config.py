...
def test_hpo_config_defaults():
    config = HPOConfig(framework_id=None, storage_uri='file:///tmp/hpo', is_continuous=None, num_trials=None, num_jobs=None)
...
def test_hpo_config_properties():
    config = HPOConfig(framework_id='optuna', storage_uri='file:///tmp/hpo', is_continuous=False, num_trials=10, num_jobs=2)
    assert config.storage_protocol == 'file'
    assert config.storage_path == '/tmp/hpo'

def test_hpo_config_get_yaml_config():
    config = HPOConfig(framework_id='optuna', storage_uri='file:///tmp/hpo', is_continuous=True, num_trials=5, num_jobs=3)
