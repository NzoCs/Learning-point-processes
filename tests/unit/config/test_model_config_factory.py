...
from easy_tpp.config_factory.base import (
    BaseConfig, ConfigValidationError, ConfigSerializationError,
    ConfigValidator, ConfigFactory, config_factory
)
from easy_tpp.config_factory.model_config import ModelConfig, ThinningConfig, SimulationConfig, TrainingConfig, ModelSpecsConfig, ModelType
...
class TestEnhancedModelConfig:
    ...
    def test_device_auto_selection_cpu(self, monkeypatch):
        ...
        monkeypatch.setattr("easy_tpp.config_factory.model_config.get_available_gpu", lambda: -1)
    ...
    def test_device_auto_selection_cuda(self, monkeypatch):
        ...
        monkeypatch.setattr("easy_tpp.config_factory.model_config.get_available_gpu", lambda: 0)
        config = ModelConfig(model_id="NHP", num_event_types=5, device="auto")
...
class TestLegacyModelConfig:
    ...
    def test_legacy_creation(self):
        ...
        config = ModelConfig(
            model_id="NHP",
            num_event_types=5,
            specs=ModelSpecsConfig(hidden_size=64)
        )
        assert config.model_id == "NHP"
        assert config.num_event_types == 5
